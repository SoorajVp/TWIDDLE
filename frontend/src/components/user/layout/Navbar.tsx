...
const Navbar = () => {
  ...
  const Navlinks = [
    ...
    { name: "Profile", href: `/${user?.name}`, icon: <img src={user?.profilePic} className="w-7 h-7 border rounded-full" alt="ProfilePic" />},
  ];
  ...
  return (
    ...
    <nav className={`${bgColor} ${color} fixed top-0 w-full border-b py-4 sm:hidden`}>
        <div className="container flex px-8 justify-between">
          ...
          <div><Link to="/messages"> <BiMessageDetail size={30} /> </Link></div> 
        </div>
      </nav>
